...
struct ArtistHeaderSection: View {
    ...
    @Query private var favorites: [FavoriteArtist]

    /// Check if current artist is a favorite
    private var isFavorite: Bool {
        favorites.contains { $0.mbid == artist.id }
    }
    ...
    private func toggleFavorite() {
        if let existing = favorites.first(where: { $0.mbid == artist.id }) {
            // Remove from favorites
            modelContext.delete(existing)
        } else {
            // Add to favorites
            let favorite = FavoriteArtist(from: artist)
